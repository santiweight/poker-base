...
holdingRangeToShapedRange :: Range Holding [PlayerActionValue] -> Range ShapedHand (Range Holding [PlayerActionValue])
holdingRangeToShapedRange (Range r) = Range $ Map.foldrWithKey combine mempty r
  where
    combine ::
      Holding ->
      [PlayerActionValue] ->
      Map ShapedHand (Range Holding [PlayerActionValue]) ->
      Map ShapedHand (Range Holding [PlayerActionValue])
    combine holding acts seed =
      seed
        & at (comboToShaped holding) . non mempty . range . at holding %~ Just . maybe acts (acts <>)

shapedRangeToFreqRange :: (PlayerActionValue -> Bool) -> Range ShapedHand (Range Holding [PlayerActionValue])
...
shapedRangeToFreqRange prop ran =
  ...
  where
    flatten :: Range Holding [PlayerActionValue] -> Range Holding (Int, Int)
...
shapeToCombos (ShapedHand (r1, r2) shape) =
  case shape of
    ...
    Suited -> do
      suit_ <- listSuit
      return $ newCombo [Card r1 suit_, Card r2 suit_]
...
findRange :: ActionIx -> RangeCollection PlayerActionValue ShapedRange -> ShapedRange
findRange indx ranC =
  let unionF = unionIndexRange indx
   in Map.foldlWithKey unionF mempty (ranC ^. ranges)

unionIndexRange :: ActionIx -> ShapedRange -> PlayerActionValue -> ShapedRange -> ShapedRange
